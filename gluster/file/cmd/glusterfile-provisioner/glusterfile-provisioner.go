...
const (
	...
	shareIDAnn         = "glusterfsShare"
	provisionerIDAnn   = "glusterfsProvIdentity"
...
type glusterfsProvisioner struct {
	...
	// Identity of this glusterfsProvisioner, generatep. Used to identify "this"
	// provisioner's PVs.
	identity string
	provisionerConfig
...
//NewglusterfsProvisionercreate a new provisioner.
...
func (p *glusterfsProvisioner) Provision(options controller.VolumeOptions) (*v1.PersistentVolume, error) {
	...
	pv.Spec.Capacity[v1.ResourceStorage] = resource.MustParse(fmt.Sprintf("%dGi", sizeGiB))

	glog.V(1).Infof("successfully created Gluster File volume %+v", pv.Spec.PersistentVolumeSource.Glusterfs)
	return pv, nil
}

func (p *glusterfsProvisioner) CreateVolume(gid *int) (r *v1.GlusterfsVolumeSource, size int, volID string, err error) {
	...
	if len(parseVolumeType) == 0 {
		...
		case "replicate":
			if len(parseVolumeTypeInfo) >= 2 {
				...
				if err != nil {
					return nil, fmt.Errorf("error [%v] when parsing value %q of option '%s' for volume plugin %s", err, parseVolumeTypeInfo[1], "volumetype", provisionerName)
				}
		...
		case "disperse":
			if len(parseVolumeTypeInfo) >= 3 {
				...
				if err != nil {
					return nil, fmt.Errorf("error [%v] when parsing value %q of option '%s' for volume plugin %s", parseVolumeTypeInfo[1], err, "volumetype", provisionerName)
				}
				newDisperseRedundancy, err := convertVolumeParam(parseVolumeTypeInfo[2])
				if err != nil {
					return nil, fmt.Errorf("error [%v] when parsing value %q of option '%s' for volume plugin %s", err, parseVolumeTypeInfo[2], "volumetype", provisionerName)
				}
